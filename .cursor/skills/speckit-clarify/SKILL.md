...
5. Sequential questioning loop (interactive):
    ...
    - For multiple‑choice questions:
       ...
       - Assign each option a **recommendation level** (推荐度):
          - `★★★` 首选 — 当前情境下的最佳选择
          - `★★` 可选 — 合理,但有代价或仅特定场景更优
          - `★` 备选 — 兜底/通常不优先
          - 多个选项可同级,但必须能区分高低;不要给所有选项相同星级。
       - **每个选项必须强制标注「依据来源」**(否则该推荐无效)。依据只能取自下列受控词表,不得编造空泛理由:
          - `[最佳实践]` / `[行业标准]` — 该类项目的通行做法
          - `[constitution]` — 来自 `.specify/memory/constitution.md` 的原则
          - `[spec 约束]` — 来自当前 spec 已写明的目标/约束
          - `[skill:<名>]` — 来自已激活维度的 skill(如 `[skill:first-principles]`、`[skill:security-and-hardening]`、`[skill:data-modeling]`)
          - `[既有栈/约定]` — 来自项目已确定的技术栈或既有代码约定
          - `[Opus 领域知识]` — **诚实兜底**:无更硬来源,仅基于模型领域判断(此时不得伪装成最佳实践)
          - 一个推荐可叠加多个来源(如 `[constitution][skill:first-principles]`)。
       - **Render options as a Markdown table, ordered by 推荐度 DESCENDING — the TOP row is the most recommended (= your recommendation).** Sort rows so stronger options appear above weaker ones; never place a weaker option above a stronger one.

       | 选项 | 推荐度 | 依据来源 | 说明 | 理由 / 取舍 |
       |------|--------|----------|------|-------------|
       | A | ★★★ | [最佳实践][skill:...] | <最推荐选项的描述> | <1 句:为什么当前最优> |
       | B | ★★ | [行业标准] | <次选描述> | <代价 / 仅何时更优> |
       | C | ★ | [Opus 领域知识] | <备选描述> | <为什么不优先>（按需加到 5 项） |
       | 其他 | — | — | 自定义短答案 (<=5 words) | 仅在自由作答合适时保留此行 |

       - 排序硬约束:行必须按推荐度从高到低;置顶行即首选,无需再单列 "Recommended" 文本。
       - 依据硬约束:除 "其他" 自定义行外,每行「依据来源」列**不得为空**;来源取自上方受控词表。
       - After the table, add: `回复选项字母(如 "A")、说 "yes"/"采纳推荐" 接受置顶首选,或直接给出你自己的短答案(<=5 words)。`
    - For short‑answer style (no meaningful discrete options):
       - Provide your **suggested answer** based on best practices and context.
       - **强制带依据来源**(受控词表同上:`[最佳实践]`/`[行业标准]`/`[constitution]`/`[spec 约束]`/`[skill:<名>]`/`[既有栈/约定]`/`[Opus 领域知识]`)。
       - Format as: `**Suggested:** <your proposed answer> — 依据: [来源标签] — <brief reasoning>`
       - Then output: `Format: Short answer (<=5 words). You can accept the suggestion by saying "yes" or "suggested", or provide your own answer.`
    - After the user answers:
       - If the user replies with "yes", "recommended", "采纳推荐", or "suggested", use the **top-ranked option** (★★★, the first table row / the suggested answer) as the answer.
